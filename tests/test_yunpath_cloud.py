...
def test_different_path(gspath):
    path1 = gspath
    path2 = YPath("gs://handy-buffer-287000.appspot.com/yunpath-test2")
    assert path1 != path2
...
def test_repr(gspath):
    ...
    assert repr(path).startswith(
        "YPath('gs://handy-buffer-287000.appspot.com/yunpath-test/"
    )
...
def test_relative_to(gspath):
    path = gspath / "test_file"
    assert path.relative_to(gspath) == YPath("test_file")
...
def test_cloud_path_comparison(gspath):
    ypath = gspath
    ppath = YPath(str(gspath))
    assert ypath == ppath
    assert isinstance(ypath, YPath)
    assert isinstance(ppath, YPath)
